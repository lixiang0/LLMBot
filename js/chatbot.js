...
ChatBot.start = function () {
    $(document).ready(function () {
        ...
        ChatBot.write("Hi,我是萌萌哒悠然机器人,欢迎访问!有什么问题你可以问我哦！", "悠然机器人");
        // ChatBot.getMessages();
        });
};
            ...
            $.post(ChatBot.SERVER_PATH + "/chat/"+ChatBot.userName, {"msg": chatInput.val()}, function (result) {
                ...
                if (typeof result != "undefined" && "msg" in result) {
                    ...
                    if (result.msg.constructor != Array){
                        ChatBot.write(result.msg, "悠然机器人");
                        }
                    else{
                        for(var i =0;i<result.msg.length;i++){
                            console.log(result.msg[i])
                            ChatBot.write(result.msg[i],"悠然机器人");
                        }
                     }
                } else {
...
ChatBot.write = function (message, sender, emoji) {
    ...
    if (sender == "me" && ChatBot.speechEnabled) {
        ChatBot.speak(message);
    }
    if (sender=='me'){
        var chatScreen = $(".chat-screen");
        // sender = $("<span />").addClass("sender").addClass("right").addClass(sender).text(sender + ":");
        var msgContent = $("<span />").addClass("msg").addClass("right").text(message);
        var newLine = $("<div />").addClass("container").addClass("darker").addClass("right");
        newLine.append(msgContent);
        ...
        var newLine = $("<div />").addClass("container");
...
ChatBot.handleServerError = function (errorThrown) {
    ...
    ChatBot.write("Sorry, there seems to be an error on the server. Let's talk later. " + actualError, "悠然机器人");
